...
public class AsyncTaskController {
    ...
    @ApiOperation(value = "testFunctionalExecutor")
    @GetMapping("func")
    public void testFunctionalExecutor() throws InterruptedException {
        ...
        for (int i = 0; i < 3; i++) {
            executeAsync.executeAsync(() -> {
                ...
                log.info(Thread.currentThread().getName() + "线程睡{}秒", random);
                Thread.sleep(random * 1000);
                latch.countDown();
                log.info(Thread.currentThread().getName() + "子线程睡{}秒,执行完毕", random);
            });
        }
        latch.await();
        log.info("主线程执行完毕");
    ...
    @ApiOperation(value = "asyncTaskTest")
    @GetMapping("func2")
